...
void AVehiclePawn::GiveBonus(int BonusType)
{


}

void AVehiclePawn::SetGrounded(bool groundedVal)
{
...
{
	if (USkeletalMeshComponent* SkeletalMeshComponent = CastChecked<USkeletalMeshComponent>(GetMesh())) {
		FVector location = SkeletalMeshComponent->GetComponentLocation();
		if (location.Z < waterLevel)
			Val /= 2;
	}
...
void AVehiclePawn::UpdateInAirControl(float DeltaTime)
{
	if (UWheeledVehicleMovementComponent4W* Vehicle4W = CastChecked<UWheeledVehicleMovementComponent4W>(GetVehicleMovement()))
	{
		...
		if (bInAir || bNotGrounded)
		{
			...
			{
				GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("In air"));
				VehicleMesh->BodyInstance.bLockXRotation = true;
				VehicleMesh->BodyInstance.bLockYRotation = true;

				if (USkeletalMeshComponent* SkeletalMeshComponent = CastChecked<USkeletalMeshComponent>(GetMesh())) {
					FRotator rotation = SkeletalMeshComponent->GetComponentRotation();
					SkeletalMeshComponent->SetAllPhysicsRotation(FRotator(0, rotation.Yaw, 0));
				}
			}
		}
		else {
			if (grounded) {
				if (USkeletalMeshComponent* SkeletalMeshComponent = CastChecked<USkeletalMeshComponent>(GetMesh()))
				{
					SkeletalMeshComponent->SetAllPhysicsRotation(FRotator(0, 0, 0));
				}
			}
		}
	}
